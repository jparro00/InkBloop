...
- **R2 migration Phase 1** — Worker at `images-dev.inkbloop.com` and `r2-upload-url` edge fn are on dev only. Prod deploy requires creating the prod R2 bucket (`inkbloop-storage`), prod R2 API token, setting `R2_*` secrets on prod Supabase, `wrangler deploy --env prod`, and `supabase functions deploy r2-upload-url --project-ref <prod-ref> --no-verify-jwt`. See [r2-migration-plan.md](./r2-migration-plan.md).
